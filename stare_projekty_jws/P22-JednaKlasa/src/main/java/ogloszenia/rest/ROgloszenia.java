...
public class ROgloszenia {
	...
	// Jeśli domyślnym formatem jest JSON, to w operacjach setCena / getCena nie trzeba zmieniać typu na tex.plain,
	// ponieważ liczba jest poprawną wartością w formacie JSON.
	@Path("/{id}/cena")
	@GET
	...
	public Sprzedawca getSprzedawca(@PathParam("id") int idOgloszenia) throws BladBazyDanych, NieznanyRekord {
		try(DostepDoBazySqlite db = DostepDoBazySqlite.newSQLite()) {
			...
			return ogloszenie.getSprzedawca();
